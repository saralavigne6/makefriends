...
{    
		 HttpSession session=request.getSession(true);  
	      ...
	      try{  
	           ...
	           if(showBean==null) 
	           {  
	              showBean=new ShowByPage();  //创建 Javabean 对象 
	      ...
	      showBean.setPageSize(3);  //每页显示 3 条记录 
	      ...
	      String url="jdbc:mysql://localhost:3306/mylogin";
			 try{  
	           ...
	           rowSet.populate(rs); 
	           con.close();                     //关闭连接 
	           showBean.setRowSet(rowSet);  //数据存储在 showBean 中 
	           rowSet.last(); 
	           int m=rowSet.getRow();           //总行数 
	           int n=pageSize; 
	           int pageAllCount=((m%n)==0)?(m/n):(m/n+1); 
	           showBean.setPageAllCount(pageAllCount);//数据存储在 showBean 中  
   ...
   {  
     ...
     { 
         ...
         ResultSet rs= 
        	     sql.executeQuery("SELECT * FROM register where logname = '"+logname+"'"); 
         if(rs.next()) 
         {  
             inform.setLogname(rs.getString(1)); 
             inform.setPhone(rs.getString(4)); 
             inform.setEmail(rs.getString(3)); 
